...
def render_rays(ray_batch,
    ...
    def raw2outputs(raw, z_vals, rays_d):
        ...
        #print(disp_map)
        has_nan = tf.reduce_any(tf.math.is_nan(disp_map))

        def print_nan_details():
            index = tf.where(tf.math.is_nan(disp_map))
            # 使用tf.gather_nd来获取索引位置的值
            disp_map_values = tf.gather_nd(disp_map, index)
            depth_map_values = tf.gather_nd(depth_map, index)
            tf.print('reduce sum:', tf.reduce_sum(weights, axis=-1))
            tf.print('disp_map indices with NaN:', index)
            tf.print('disp_map values at those indices:', disp_map_values)
            tf.print('depth_map values at those indices:', depth_map_values)
            return tf.constant(True)

        # 如果在Eager执行模式下，可以直接评估has_nan的值
        if has_nan.numpy():
            print_nan_details()

        #print(disp_map[tf.where(tf.math.is_nan(disp_map))])
...
def create_nerf(args):
    ...
    model = init_nerf_model(
        ...
        input_ch_views=input_ch_views, use_viewdirs=args.use_viewdirs, transfer=args.transfer)
...
def config_parser():
    ...
    parser.add_argument("--N_iters", type=int,
                        default=10005, help='iterate number')
    parser.add_argument("--transfer", type=int,
                        default=0, help='whether transfer learning')
    return parser
...
def train():
    ...
    if use_batching:
        ...
        print('rays shape:', rays_rgb.shape, rays_rgb[0])
        # breakpoint()
        print('done')
        i_batch = 0

    N_iters = args.N_iters#10005#000
    print('Begin', N_iters)
